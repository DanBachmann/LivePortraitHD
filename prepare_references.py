...
def extract_semantic_square(image, landmarks, feature_indices, output_size=512, padding_factor=1.2):
    ...
    # 1. Extract pixel coordinates for all boundary indices
    points = []
    ...
    # 2. Find the absolute bounding box of the feature
    min_x, min_y = np.min(points, axis=0)
    max_x, max_y = np.max(points, axis=0)
    
    # 3. Calculate the center and dynamically size the square
    ...
    # 4. Calculate crop coordinates bounded by the image dimensions
    ...
    # 5. Pad with black if the bounding box hit the physical edge of the source image
    ...
    # 6. scale the tight crop to the IP-Adapter requirement using high-frequency preservation
